  ...
  factory: () => {
    ...
    module.exports.apply = (ctx) => {
      ...
      const FIELD_SPECS = {
        "firecrawl-keyless": [],
        tavily: [
          { key: "searchDepth", type: "select", label: "搜索深度", options: ["basic", "advanced", "fast", "ultra-fast"], default: "basic",
            hint: "basic 快而省（1 credit）；advanced 深挖更多来源并重排（约 2 credits）；fast / ultra-fast 为低延迟档。" },
          { key: "topic", type: "select", label: "话题类别", options: ["general", "news", "finance"], default: "general",
            hint: "general 常规网页；news 新闻源、时效优先；finance 财经数据源。" },
          { key: "maxResults", type: "number", label: "单次返回结果数", default: 5,
            hint: "单次搜索返回的结果条数上限。" },
          { key: "includeAnswer", type: "bool", label: "附带摘要回答", default: false,
            hint: "由后端在结果之外再生成一段摘要回答。" },
          { key: "timeRange", type: "text", label: "时间范围", default: "",
            hint: "限制结果发布时间：day / week / month / year，留空不限。" },
          { key: "extractDepth", type: "select", label: "抽取深度", options: ["basic", "advanced"], default: "basic",
            hint: "basic 常规抽取；advanced 提取更完整（含页面嵌入内容），更慢、更耗配额。" },
          { key: "researchModel", type: "select", label: "Research 模型", options: ["auto", "mini", "pro"], default: "auto",
            hint: "research 异步任务所用模型：auto 自选 / mini 快 / pro 强。" },
        ],
        deepseek: [
          ...
          { key: "maxTokens", type: "number", label: "单次最大 Token", default: 4096,
            hint: "每次服务端搜索调用的响应 Token 上限，影响成本与截断点。" },
          { key: "maxUses", type: "number", label: "单次请求最多搜索次数", default: 5,
            hint: "单次对话内允许工具执行的搜索次数上限。" },
        ],
      };
