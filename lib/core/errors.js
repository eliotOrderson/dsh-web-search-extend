...
export function credentialMissing(ref) {
    return new WebError(`Search has no API key for "${ref}"; store it through the credentials service (the web Models page writes it), export it in the launching environment, or set a literal "apiKey" in the web-search-extend config`, "WEB_PROVIDER_CREDENTIAL_MISSING");
}
