...
/** DeepSeek-specific settings read from `config.deepseek`. */
...
function citationSnippets(blocks: any[]): Map<string, string> {
 ...
 */
function mapDeepSeekResponse(response: any): WebSearchResult {
	const blocks = response?.content ?? [];
	const resultBlocks = blocks.filter((block: any) => block.type === "web_search_tool_result");
	...
	for (const block of resultBlocks) {
		for (const item of block.content ?? []) {
			if (item.type !== "web_search_result" || item.url == null || item.url.length === 0 || seen.has(item.url)) continue;
			seen.add(item.url);
...
export const DeepSeekAdapter: SearchAdapter = {
	id: "deepseek",
	...
	async search(request: Parameters<SearchAdapter["search"]>[0], runtime: AdapterRuntime, signal?: AbortSignal): Promise<WebSearchResult> {
		...
		const settings = runtime.settings as DeepSeekSettings;
