...
import { ExtensibleWebSearchProvider, type ResolvedOptions } from "./core/provider.js";
...
function resolveOptions(ctx: Context, getConfig: () => ConfigType, registry: AdapterRegistry): () => ResolvedOptions {
	return () => {
		...
		const settings = ((config as Record<string, unknown>)[provider] ?? {}) as Record<string, unknown>;
		return {
			provider,
