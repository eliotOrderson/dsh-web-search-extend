...
| Tag | dsh version |
| :-- | :---------- |
| `v0.2.3` | **dsh 0.1.5-rc.1 and later** — install-robustness fix over `v0.2.2`: the Firecrawl SDK is bundled into `lib/index.js` together with the `zod` / `zod-to-json-schema` pair it was built against, so the plugin installs no Firecrawl or zod package into the profile at all |
...
`v0.2.3` supersedes that dependency-level fix, which only held when the profile resolved fresh: a
profile whose lock already hoisted `zod@3.23.8` keeps it at the root, so the hoisted
`zod-to-json-schema` imported `zod/v3` from that copy and the entry failed again. The host build now
inlines the Firecrawl SDK with the `zod` / `zod-to-json-schema` pair it was built against (both are
build-time devDependencies), so profile hoisting cannot reach the plugin's SDK path anymore.
...
(model / base URL / API version / max tokens / max uses) from its exports, so the takeover
follows official updates instead of mirroring them. The package ships with the harness — keep
it installed.
...
| :--- | :--- | :--- | :--- |
| `firecrawl-keyless` (**default**) | `FIRECRAWL_API_KEY` (optional; upgrades quota) | search out of the box; the agent-style research endpoint refuses keyless (HTTP 401); free tier ~1000 credits/month/IP, HTTP 402 when exhausted | all five (search/extract/crawl/map/research) |
...
`FirecrawlKeylessAdapter` is the default `provider`: all five ops run natively
against the hosted Firecrawl v2 endpoint - search works with zero configuration. Caveats:
...
- **Research needs a key** - the agent-style research endpoint refuses the keyless
  free tier (HTTP 401 → `WEB_PROVIDER_ERROR` naming `FIRECRAWL_API_KEY`);
  search/extract/crawl/map share the monthly credit pool.
...
src/
  ...
  config.ts           # config layer: superset schema; DeepSeek defaults inherited from official exports
...
- **127 vitest tests** (`tests/`): router ladder, capability pinning (tavily/firecrawl: all five
  ...
  402/429 quota/rate-limit errors, research-keyless 401), ChainAdapter failover (switchable vs non-switchable,
